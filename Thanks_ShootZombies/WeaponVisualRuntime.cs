...
public static partial class ItemPatch
{
	...
	public static bool EnsureAkVisual(Item item, bool forceRefreshMarker = false)
	{
		...
		BlowgunInfiniteUsePatch.EnsureInfiniteUse(item);
		ApplyAkDisplay(item);
	...
	public static void ApplyAkDisplay(Item item)
	{
		...
		{
			Plugin.TryProtectWeaponItemRuntime(item);
	...
	{
		if (Plugin.IsWeaponFeatureEnabled() && !((Object)item == (Object)null) && IsBlowgunLike(item))
		{
			ApplyAkDisplay(item);
		}
	}
	...
	public static bool IsBlowgunLike(Item item, string knownDisplayName = null)
	{
		...
		if (BlowgunInfiniteUsePatch.IsBlowgunItem(item))
		{
			return true;
		}
